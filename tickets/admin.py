...
from .models import Announcement, Ticket, TicketAttachment, TicketCategory, TicketEvent, User


@admin.register(User)
class UserAdmin(BaseUserAdmin):
    model = User
    list_display = ("matricula", "full_name", "email", "vinculo", "area", "cargo", "is_staff")
    search_fields = ("matricula", "full_name", "email")
    ordering = ("full_name",)
    fieldsets = (
        (None, {"fields": ("matricula", "email", "password")}),
        ("Dados pessoais", {"fields": ("full_name", "vinculo", "area", "cargo", "phone")}),
    ...
    add_fieldsets = (
        (None, {
            "classes": ("wide",),
            "fields": ("matricula", "email", "full_name", "vinculo", "area", "cargo", "password1", "password2"),
        }),
    )
...
class TicketAdmin(admin.ModelAdmin):
    list_display = ("id", "title", "department", "status", "requester", "assigned_to", "opened_at")
    list_filter = ("department", "status", "urgency")
